    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 relative z-10">
          <div className="w-24 h-24 rounded-full bg-slate-800 flex items-center justify-center text-3xl font-bold text-slate-300 border-4 border-slate-950 shadow-xl">
            JC
          </div>
          ...
          <div className="flex gap-3">
             <button 
              ...
              className="bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 px-4 py-2 rounded-lg font-medium flex items-center gap-2 transition-colors border border-emerald-500/20"
             >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
            ...
            <div className="space-y-3">
              <button 
                ...
                className="w-full flex items-center justify-between p-3 rounded-lg bg-slate-950 border border-slate-800 hover:border-slate-700 transition-colors group"
              >
              ...
              <button 
                ...
                className="w-full flex items-center justify-between p-3 rounded-lg bg-slate-950 border border-slate-800 hover:border-slate-700 transition-colors group"
              >
                <span className="text-slate-300 font-medium group-hover:text-white">Request Overtime</span>
                <span className="text-slate-500">&rarr;</span>
              </button>
              <button 
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
            ...
            <div className="space-y-4 relative before:absolute before:inset-0 before:ml-2 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-slate-800 hidden md:block">
               {/* Just a simple list instead of timeline for simpler layout */}
            </div>
            <div className="space-y-3">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl relative">
            ...
            <div className="text-center mb-6">
              <h3 className="text-lg font-bold text-white">Your Production QR</h3>
              <p className="text-sm text-slate-400">Scan this at the terminal to log your work</p>
            </div>
            <div className="bg-white p-4 rounded-xl flex items-center justify-center mb-6">
              {/* Fake QR Code Pattern using generic CSS grid */}
              <div className="w-48 h-48 grid grid-cols-6 grid-rows-6 gap-1 bg-white p-2">
                {[...Array(36)].map((_, i) => (
                  <div key={i} className={`bg-black rounded-sm ${Math.random() > 0.4 ? 'opacity-100' : 'opacity-0'}`}></div>
                ))}
              </div>
            </div>
            <p className="text-center text-emerald-400 font-mono font-medium tracking-widest text-lg">EMP-001</p>
          </div>
        </div>
      )}
    </div>
