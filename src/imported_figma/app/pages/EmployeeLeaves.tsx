...
import { Plus, Calendar as CalendarIcon, Clock, CheckCircle2, XCircle, Clock4 } from "lucide-react";
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex bg-slate-900 border border-slate-800 rounded-lg p-1">
          ...
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${
              activeTab === "new" ? "bg-emerald-500 text-white" : "text-slate-400 hover:text-slate-200"
            }`}
          >
            New Request
          </button>
                  ...
                  <tr key={req.id} className="hover:bg-slate-800/20 transition-colors text-slate-300">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium border ${
                        ...
                        {req.type === "Leave" ? <CalendarIcon className="w-3.5 h-3.5" /> : <Clock className="w-3.5 h-3.5" />}
                        {req.type}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      {req.date}
                      {req.duration && <span className="block text-xs text-slate-500 mt-0.5">{req.duration}</span>}
                    </td>
                    <td className="px-6 py-4">{req.reason}</td>
                    <td className="px-6 py-4 text-right">
                      {req.status === "Approved" && <span className="inline-flex items-center gap-1 text-emerald-400"><CheckCircle2 className="w-4 h-4"/> Approved</span>}
                      {req.status === "Pending" && <span className="inline-flex items-center gap-1 text-amber-400"><Clock4 className="w-4 h-4"/> Pending</span>}
                      {req.status === "Rejected" && <span className="inline-flex items-center gap-1 text-rose-400"><XCircle className="w-4 h-4"/> Rejected</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 sm:p-8 max-w-2xl">
          <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setActiveTab("history"); }}>
            ...
            <div className="space-y-2">
              ...
              <textarea 
                rows={4} 
                className="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-emerald-500 resize-none"
            ...
            <div className="pt-2 flex justify-end gap-3">
              ...
              <button 
                type="submit"
                className="px-5 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium rounded-lg transition-colors shadow-lg shadow-emerald-500/20 flex items-center gap-2"
              >
