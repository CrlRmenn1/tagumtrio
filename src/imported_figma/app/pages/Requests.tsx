  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        <button className="bg-slate-900 border border-emerald-500/30 px-4 py-2 rounded-lg text-emerald-400 text-sm font-medium hover:bg-emerald-500/10 transition-colors">
          All Requests
        </button>
        <button className="bg-slate-900 border border-slate-800 px-4 py-2 rounded-lg text-slate-400 text-sm font-medium hover:bg-slate-800 transition-colors">
          Leaves
        </button>
        <button className="bg-slate-900 border border-slate-800 px-4 py-2 rounded-lg text-slate-400 text-sm font-medium hover:bg-slate-800 transition-colors">
          Transfers
          ...
          <div key={req.id} className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
            <div className="flex gap-4">
              <div className={`p-3 rounded-xl h-fit ${
                req.type === 'Leave' ? 'bg-rose-500/10 text-rose-400' :
                req.type === 'Transfer' ? 'bg-blue-500/10 text-blue-400' :
                'bg-emerald-500/10 text-emerald-400'
              }`}>
                {req.type === 'Leave' ? <FileText className="w-6 h-6" /> :
                 req.type === 'Transfer' ? <ArrowRightLeft className="w-6 h-6" /> :
              ...
              <div>
                ...
                <p className="text-sm text-slate-300 mt-2 bg-slate-950 p-2 rounded border border-slate-800/50">
                  {req.reason}
                </p>
              </div>
            </div>
            ...
            {req.status === 'Pending' && (
              <div className="flex gap-2 w-full md:w-auto mt-4 md:mt-0">
                <button className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 px-4 py-2 rounded-lg text-sm font-medium transition-colors border border-emerald-500/20">
                  <CheckCircle className="w-4 h-4" />
                  Approve
                </button>
                <button className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 px-4 py-2 rounded-lg text-sm font-medium transition-colors border border-rose-500/20">
