...
export default function ProductionDashboard() {
	...
	function handleScan() {
		submitScan({ workerId: 'EMP-001', workerName: 'Juan Dela Cruz', product: 'Marine Plywood 3/4"', pieces: 25 })
		setShowScan(true)
		setTimeout(() => setShowScan(false), 1500)
	}
	...
	return (
		<div className="space-y-6">
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
				...
				<div className="text-sm text-slate-400">Scanning is performed by employees using the Employee Portal (mobile). Leadman and Head will verify submitted scans.</div>
			</div>
						...
						<tbody>
							{productionLogs.map((log) => (
								<tr key={log.id} className="border-b border-slate-800 hover:bg-slate-800/30 transition-colors">
									...
									<td className="p-4">
										<span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${log.status === 'Verified' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400'}`}>
											{log.status}
										</span>
									</td>
								</tr>
