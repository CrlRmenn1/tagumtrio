        ...
        {MOCK_PAYSLIPS.map((payslip) => (
          <div key={payslip.id} className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-col sm:flex-row gap-4 justify-between group hover:border-slate-700 transition-colors">
            <div className="flex gap-4 items-start">
              ...
              <button onClick={() => navigate(`/app/portal/payslips/${payslip.id}`)} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-sm text-slate-300 hover:text-white hover:border-slate-700 transition-colors"><Eye className="w-4 h-4"/> View</button>
              <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-3 py-2 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-lg text-sm hover:bg-emerald-500/20 transition-colors"><Download className="w-4 h-4"/> PDF</button>
            </div>
          </div>
        ))}
      </div>
